...
aca[,aca_15_16_chg := (aca_plans_2016 - aca_plans_2015) / aca_plans_2016]
...
### join data --------------------
lapply(list(demographics, aca, election), setkeyv, "fips")

joined = aca[election]
joined = demographics[joined]
state_totals = joined[,list(county_votes = max(total_votes)), by = st]
joined = merge(joined, state_totals, by = "st", all = TRUE)
joined[,":="(pct_plans_15 = aca_plans_2015 / total_pop,
             pct_plans_16 = aca_plans_2016 / total_pop)]
joined = joined[cand == "Donald Trump"]

```

```{r plot relationships, include = FALSE}
library(ggplot2)
ggplot(data = joined, aes(x = pct, y = aca_15_16_chg)) + geom_point() +
  geom_smooth()

ggplot(data = joined, aes(x = as.character(ifelse(pct >= .5, 1, 0)), y = aca_15_16_chg)) + geom_boxplot()
